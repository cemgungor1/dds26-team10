...
TPC_RECOVERY_INTERVAL = float(os.environ.get("SAGA_RECOVERY_INTERVAL", "5"))
TPC_STALE_THRESHOLD = float(os.environ.get("SAGA_STALE_THRESHOLD", "8"))
...
def _build_tpc_start(order_id: str, order_entry: OrderValue) -> tuple[dict, dict, dict]:
    ...
    state = {
        ...
        "decision": "",
        "reason": "",
        "votes": {"stock": None, "payment": None, "stock_reason": "", "payment_reason": ""},
        "acks": {"stock": False, "payment": False},
...
def _tpc_send_decision_commands(order_id: str, state: dict) -> None:
    ...
    if decision == "commit":
        stock_cmd = {
            "message_id": str(uuid.uuid4()), "type": "CommitStock",
            "order_id": order_id, "transaction_id": transaction_id,
            "attempt_id": attempt_id, "items": state["items"],
            "idempotency_key": state["stock_decision_idem"],
        }
        payment_cmd = {
            "message_id": str(uuid.uuid4()), "type": "CommitPayment",
            "order_id": order_id, "transaction_id": transaction_id,
            "attempt_id": attempt_id, "user_id": state["user_id"],
            "amount": state["total_cost"],
            "idempotency_key": state["payment_decision_idem"],
        }
    else:
        stock_cmd = {
            "message_id": str(uuid.uuid4()), "type": "AbortStock",
            "order_id": order_id, "transaction_id": transaction_id,
            "attempt_id": attempt_id, "items": state["items"],
            "idempotency_key": state["stock_decision_idem"],
        }
        payment_cmd = {
            "message_id": str(uuid.uuid4()), "type": "AbortPayment",
            "order_id": order_id, "transaction_id": transaction_id,
            "attempt_id": attempt_id, "user_id": state["user_id"],
            "amount": state["total_cost"],
            "idempotency_key": state["payment_decision_idem"],
...
def _start_tpc(order_id: str) -> tuple[bool, str]:
    ...
    # FIX C1 (partial) / same as B6 in order-2pc-app: if either prepare send
    # failed, abort both participants immediately rather than leaving locks
    # dangling until the recovery loop fires.
    logger.warning(
        "TPC %s: partial prepare send failure (stock_sent=%s, payment_sent=%s); aborting",
        state["transaction_id"], stock_sent, payment_sent,
    )
    state["decision"] = "abort"
    state["reason"] = "Failed to send all prepare commands"
    _tpc_send_decision_commands(order_id, state)  # sends AbortStock + AbortPayment
    state["status"] = "failed"
    state["step"] = "done"
...
def _handle_tpc_decision_ack(state: dict, event: dict) -> None:
    ...
    if decision == "commit":
        ...
        # FIX C3: Mirror of B7 in order-2pc-app.  The original checked
        # `state["acks"]["stock"] and state["acks"]["payment"]` which is only
        # True when both are True.  A False ack was silently stored and the
        # completion check never fired, leaving the transaction in "committing"
        # indefinitely.  Now we log the failure explicitly and return early so
        # the recovery loop can re-drive the commit command on the next scan.
        if not success:
            logger.error(
                "TPC %s: %s returned success=False (%s); "
                "staying in committing state for recovery re-drive",
                state["transaction_id"], event_type, reason,
            )
            return
...
def _recover_tpc(order_id: str, state: dict) -> None:
    ...
    if step == "wait_prepare":
        stock_prepare = {
            ...
            "attempt_id": state.get("attempt_id", ""), "items": state["items"],
            "idempotency_key": state["stock_prepare_idem"],
        }
        payment_prepare = {
            "message_id": str(uuid.uuid4()), "type": "PreparePayment",
            "order_id": order_id, "transaction_id": state["transaction_id"],
            "attempt_id": state.get("attempt_id", ""), "user_id": state["user_id"],
            "amount": state["total_cost"],
            "idempotency_key": state["payment_prepare_idem"],
        }
...
def _tpc_recovery_loop() -> None:
    if not KAFKA_AVAILABLE:
        return
    while True:
        try:
            time.sleep(TPC_RECOVERY_INTERVAL)
            now = time.time()
            for key in db.scan_iter(match=f"{TPC_STATE_PREFIX}*", count=100):
                try:
                    ...
                    if state.get("status") in ("completed", "failed"):
                        continue
                    if now - state.get("last_updated", 0) > TPC_STALE_THRESHOLD:
                        order_id = state.get("order_id")
                        if order_id:
                            logger.info("Recovering stale TPC for order %s at step %s",
                                        order_id, state.get("step"))
                            _recover_tpc(order_id, state)
                except Exception:
                    logger.exception("Error recovering TPC from key %s", key)
        except Exception:
            logger.exception("TPC recovery loop error")
            time.sleep(TPC_RECOVERY_INTERVAL)

def _start_tpc_recovery() -> None:
    if not KAFKA_AVAILABLE:
        return
    thread = threading.Thread(target=_tpc_recovery_loop, daemon=True)
    thread.start()
    ...
    while True:
        try:
            consumer = KafkaConsumer(
                ...
                group_id="orchestrator-2pc-events",
            ...
            for message in consumer:
                try:
                    ...
                    logger.exception("Failed to handle 2PC event")
        except NoBrokersAvailable:
            logger.warning("Kafka unavailable for 2PC event consumer; retrying in 1s")
            time.sleep(1.0)
        except Exception:
            logger.exception("2PC event consumer failed; retrying in 1s")
            time.sleep(1.0)
...
def checkout_kafka_2pc(order_id: str) -> Response:
    order_entry_raw = None
    try:
        order_entry_raw = db.get(order_id)
    except redis.exceptions.RedisError:
        abort(400, DB_ERROR_STR)

    if not order_entry_raw:
        abort(400, f"Order: {order_id} not found!")

    try:
        order_entry = msgpack.decode(order_entry_raw, type=OrderValue)
    except Exception:
        abort(400, DB_ERROR_STR)
...
def handle_event(event: dict) -> bool:
    """Entry point called by orchestrator-app.py event consumer."""
    return _handle_tpc_event(event)

def checkout(order_id: str) -> Response:
    """Entry point called by orchestrator-app.py /transactions handler."""
    return checkout_kafka_2pc(order_id)

def start_transaction(body: dict) -> Response:
    """Entry point called by orchestrator-app.py POST /transactions."""
